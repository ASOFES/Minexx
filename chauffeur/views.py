    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if mission.kilometrage_fin is not None and mission.vehicule and (
                ...
                or mission.vehicule.kilometrage_actuel < mission.kilometrage_fin
            ):
...
def missions_excel(request):
    ...
    try:
        ...
        headers = [
            ...
            "Motif", "Nombre de passagers", "Statut", "Chauffeur", "Véhicule",
            "Châssis", "Moteur", "Carte rose", "Km début véhicule", "Pneus",
            "Dispatcher", "Date départ", "Date arrivée", "Km départ", "Km arrivée", "Distance parcourue",
        ]
        ...
        for row, mission in enumerate(missions, start=2):
            try:
                ...
                ws.cell(row=row, column=11, value=mission.vehicule.numero_chassis if mission.vehicule else "")
                ws.cell(row=row, column=12, value=mission.vehicule.numero_moteur if mission.vehicule else "")
                ws.cell(row=row, column=13, value=mission.vehicule.numero_carte_rose if mission.vehicule else "")
                ws.cell(row=row, column=14, value=mission.vehicule.kilometrage_debut if mission.vehicule and mission.vehicule.kilometrage_debut is not None else "")
                ws.cell(row=row, column=15, value=mission.vehicule.pneus_resume() if mission.vehicule else "")
                ws.cell(row=row, column=16, value=mission.dispatcher.get_full_name() if mission.dispatcher else "")
                ws.cell(row=row, column=17, value=mission.date_depart.strftime("%d/%m/%Y %H:%M") if mission.date_depart else "")
                ws.cell(row=row, column=18, value=mission.date_fin.strftime("%d/%m/%Y %H:%M") if mission.date_fin else "")
                ws.cell(row=row, column=19, value=mission.kilometrage_depart or "")
                ws.cell(row=row, column=20, value=mission.kilometrage_fin or "")
                ws.cell(row=row, column=21, value=mission.distance_parcourue or "")
            except Exception as e:
...
def mission_excel(request, mission_id):
    ...
    data = [
        ...
        ("Châssis", mission.vehicule.numero_chassis if mission.vehicule else ""),
        ("Moteur", mission.vehicule.numero_moteur if mission.vehicule else ""),
        ("Carte rose", mission.vehicule.numero_carte_rose if mission.vehicule else ""),
        ("Km début véhicule", mission.vehicule.kilometrage_debut if mission.vehicule and mission.vehicule.kilometrage_debut is not None else ""),
        ("Pneus", mission.vehicule.pneus_resume() if mission.vehicule else ""),
...
def rapport_chauffeur(request):
    ...
    jours_travailles = missions.values('date_depart__date').distinct().count()
    missions_terminees = missions.filter(statut='terminee').count()
    conso_moyenne = None
    cout_total = 0
    if total_km > 0:
        # Consommation sur ravitaillements du chauffeur alignés sur la période
...
def rapport_demandeur(request):
    ...
    date_debut = request.GET.get('date_debut')
    date_fin = request.GET.get('date_fin')
    # On filtre les missions avec distance connue (pas de plafond artificiel)
    missions = Course.objects.filter(Q(distance_parcourue__isnull=True) | Q(distance_parcourue__gt=0))
    if user.role == 'admin' and not user.is_superuser and user.etablissement_id:
        missions = missions.filter(
            Q(demandeur__etablissement_id=user.etablissement_id)
            | Q(vehicule__etablissement_id=user.etablissement_id)
        )
    if demandeur:
    ...
    if date_fin:
        missions = missions.filter(date_depart__date__lte=date_fin)
    missions = missions.order_by('-date_depart')
    ...
    # Coût réel = carburant + entretien des véhicules concernés / km (si km > 0)
    from rapport.report_utils import vehicle_expenses, cost_per_km
    vehicule_ids = list(missions.exclude(vehicule_id__isnull=True).values_list('vehicule_id', flat=True).distinct())
    depenses = 0.0
    for vid in vehicule_ids:
        depenses += vehicle_expenses(vid, date_debut, date_fin).get('cout_total', 0) or 0
    cout_par_km = cost_per_km(depenses, total_km)
    cout_total_estime = depenses
...
def rapport_demandeur_pdf(request):
    ...
    try:
        from rapport.report_utils import vehicle_expenses, cost_per_km
        vids = list(missions.exclude(vehicule_id__isnull=True).values_list('vehicule_id', flat=True).distinct())
        depenses = sum(vehicle_expenses(vid, date_debut, date_fin).get('cout_total', 0) or 0 for vid in vids)
        cout_par_km = cost_per_km(depenses, total_km)
        cout_total_estime = depenses
    except Exception:
        pass
    ...
    from django.db.models.functions import TruncMonth
    stats_mois = missions.annotate(mois=TruncMonth('date_depart')).values('mois').annotate(
        missions=Count('id'),
        km=Sum('distance_parcourue'),
    ).order_by('mois')
